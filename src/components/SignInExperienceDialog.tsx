
import React, { useEffect, useState } from "react";
import { useNavigate } from 'react-router-dom';
import {
  Dialog,
  DialogContent,
  DialogHeader,
  DialogTitle,
  DialogDescription,
} from "@/components/ui/dialog";
import { Button } from "@/components/ui/button";
import { UserPlus, X, Sparkles, FileText, Clock, Github, Brain, Wand2, Zap } from "lucide-react";

const SESSION_KEY = "md-studio-signin-dialog-dismissed";

export function SignInExperienceDialog() {
  const [open, setOpen] = useState(false);
  const navigate = useNavigate();

  useEffect(() => {
    // Only show if they haven't dismissed it this session
    const dismissed = sessionStorage.getItem(SESSION_KEY);
    if (!dismissed) setOpen(true);
  }, []);

  const handleDismiss = () => {
    setOpen(false);
    sessionStorage.setItem(SESSION_KEY, "1");
  };

  const handleSignIn = () => {
    navigate('/auth');
  };

  return (
    <Dialog open={open} onOpenChange={handleDismiss}>
      <DialogContent className="sm:max-w-md max-w-[90vw] p-0 gap-0 overflow-hidden">
        {/* Close Button */}
        <button
          onClick={handleDismiss}
          className="absolute right-4 top-4 z-10 rounded-sm opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2"
        >
          <X className="h-4 w-4" />
          <span className="sr-only">Close</span>
        </button>

        {/* Header */}
        <DialogHeader className="px-6 pt-8 pb-4 text-center">
          <DialogTitle className="text-xl font-semibold">
            Unlock AI-Powered Features
          </DialogTitle>
          <DialogDescription className="text-sm text-muted-foreground mt-2">
            Sign in for free to access the complete AI writing suite
          </DialogDescription>
        </DialogHeader>

        {/* AI Features Showcase */}
        <div className="px-6 pb-4">
          <div className="space-y-3">
            <div className="flex items-center gap-3 p-3 rounded-lg bg-gradient-to-r from-primary/10 to-primary/5 border border-primary/20">
              <div className="p-2 rounded-full bg-primary/20">
                <Sparkles className="w-4 h-4 text-primary" />
              </div>
              <div>
                <p className="font-medium text-sm">AI Format & Enhancement</p>
                <p className="text-xs text-muted-foreground">Intelligent markdown formatting with grammar improvements</p>
              </div>
            </div>
            
            <div className="flex items-center gap-3 p-3 rounded-lg bg-muted/50">
              <div className="p-2 rounded-full bg-primary/10">
                <Wand2 className="w-4 h-4 text-primary" />
              </div>
              <div>
                <p className="font-medium text-sm">Smart Templates</p>
                <p className="text-xs text-muted-foreground">Quick-insert templates for tables, code blocks, and more</p>
              </div>
            </div>
            
            <div className="flex items-center gap-3 p-3 rounded-lg bg-muted/50">
              <div className="p-2 rounded-full bg-primary/10">
                <Brain className="w-4 h-4 text-primary" />
              </div>
              <div>
                <p className="font-medium text-sm">Smart Text Selection</p>
                <p className="text-xs text-muted-foreground">Context-aware formatting suggestions as you write</p>
              </div>
            </div>
            
            <div className="flex items-center gap-3 p-3 rounded-lg bg-muted/50">
              <div className="p-2 rounded-full bg-primary/10">
                <FileText className="w-4 h-4 text-primary" />
              </div>
              <div>
                <p className="font-medium text-sm">Document Management</p>
                <p className="text-xs text-muted-foreground">Save, organize, and sync your documents across devices</p>
              </div>
            </div>
            
            <div className="flex items-center gap-3 p-3 rounded-lg bg-muted/50">
              <div className="p-2 rounded-full bg-primary/10">
                <Clock className="w-4 h-4 text-primary" />
              </div>
              <div>
                <p className="font-medium text-sm">Version History</p>
                <p className="text-xs text-muted-foreground">Track changes and restore previous versions</p>
              </div>
            </div>
          </div>
        </div>

        {/* Sign In Options */}
        <div className="px-6 pb-6">
          <div className="space-y-3">
            <Button
              onClick={handleSignIn}
              className="w-full bg-gradient-to-r from-primary to-primary/80 hover:from-primary/90 hover:to-primary/70 text-primary-foreground"
              size="sm"
            >
              <Zap className="w-4 h-4 mr-2" />
              Sign In with Email
            </Button>
            
            <div className="grid grid-cols-2 gap-2">
              <Button
                onClick={handleSignIn}
                variant="outline"
                className="w-full text-xs"
                size="sm"
              >
                <svg className="w-3 h-3 mr-1" viewBox="0 0 24 24">
                  <path fill="currentColor" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
                  <path fill="currentColor" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
                  <path fill="currentColor" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
                  <path fill="currentColor" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
                </svg>
                Google
              </Button>
              <Button
                onClick={handleSignIn}
                variant="outline"
                className="w-full text-xs"
                size="sm"
              >
                <Github className="w-3 h-3 mr-1" />
                GitHub
              </Button>
            </div>
            
            <Button
              onClick={handleDismiss}
              variant="ghost"
              className="w-full text-muted-foreground hover:text-foreground"
              size="sm"
            >
              Continue as Guest
            </Button>
          </div>
        </div>
      </DialogContent>
    </Dialog>
  );
}
